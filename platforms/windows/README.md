...
- **Mini window**: `adhush run` starts `adhush overlay` — a small, borderless,
  ...
  library only; tkinter ships with the python.org installer. `[ui] overlay = false` or `--no-overlay` disables it.
...
- **Capture**: `screen` (gdigrab), `camera` (dshow), or `microphone` (`audio_device = "dshow:audio=Microphone (…)"`).
- **Control**: `local_audio` (needs `nircmd` on PATH), `ir_blaster_net`, or `network_ip`.
