...
def test_av_replay_detects_both_ad_pods(tmp_path: Path) -> None:
    ...
    assert {d.name for d in detectors} == {"black_frame", "silence", "loudness", "scene_cut", "clock", "jingle", "aspect_change", "rating_bug", "ad_units", "cutscene", "stereo_width", "watermark", "schedule", "scte35", "crowd"}
    assert len(labels) == 2
...
def test_audio_only_replay_still_works_with_reduced_set(tmp_path: Path) -> None:
    ...
    assert {d.name for d in detectors} == {"silence", "loudness", "clock", "jingle", "ad_units", "stereo_width", "schedule", "scte35", "crowd"}
