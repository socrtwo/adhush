# Release 0.28.4 — what runs where
...
|---|---|---|---|---|
| **Windows 10/11 (x64)** | yes | `adhush service install` (Task Scheduler, at logon; uses the silent `adhushw.exe`) | `adhush overlay` (Tk) — also the Mini window button in Chrome/Edge | `adhush-0.28.4-windows-x64.zip`; or the wheel |
| **macOS (Apple silicon)** | yes | `adhush service install` (launchd agent) | `adhush overlay` (Tk); Mini window in Chrome/Edge | `adhush-0.28.4-macos-arm64.zip`; Intel Macs: the wheel with python.org Python |
| **Linux desktop (x64 / arm64)** | yes (reference) | `adhush service install` (systemd --user) | `adhush overlay` (Tk; `apt install python3-tk` if the overlay says so) | `adhush-0.28.4-linux-x64.tar.gz` / `-linux-arm64.tar.gz`; needs glibc 2.35+ (Ubuntu 22.04, Debian bookworm) |
| **Raspberry Pi 4 / 5** | yes (reference) | `adhush service install`; headless box: `scripts/install-pi.sh` | n/a | `adhush-0.28.4-raspberry-pi.zip` — the arm64 binary, the listener config and the PDF guides; 64-bit Pi OS bookworm |
...
| **Web** | no — thin client | n/a | Mini window (Document PiP in Chromium 116+); popup elsewhere | open `http://<core>:8675/` — the core serves it; the page itself is `adhush-web-0.28.4.zip` |
| **Android** | **yes — on-device app** (since 0.7.0; ten methods, stream learning, the remote, the set-up wizard, any brand of TV) | microphone foreground service | notification + Quick Settings tile | `adhush-0.28.4-android-debug.apk`, sideloaded; or the web app |
...
Buttons show real outcomes only: busy while a job runs, then green or
red; model downloads verified, resumable, never installed short.

## New in 0.28.3

Every Android button dips when pressed and turns green or red with the
outcome; the teach and duck buttons are bright only when they would act.

## New in 0.28.2

Stream learning names a player that blocks capture within eight seconds
and says what to do instead.

## New in 0.28.1

The learning loop of the first evening closed (ADR 0027, amended): short
ducks teach nothing, one session counts once, a slip of the finger is not
material, a false match forgets itself. On the phone: Forget what it
learned, Save the log to a file, the wizard as a Home-page button, and a
remote laid out like one.

## New in 0.28.0

Jingle families (a sting re-cut a few semitones up or a little slower is
the same sting), the segment stinger detector, and the hour of day on
every learned jingle (ADR 0027). Both cores.

## New in 0.27.0

Android TV / Google TV, Hisense VIDAA and Philips JointSpace from the
phone (ADR 0026), each paired once through the wizard.
