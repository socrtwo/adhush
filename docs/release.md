...
|---|---|---|---|---|
| **Windows** | yes | `adhush service install` (Task Scheduler, at logon) | `adhush overlay` (Tk) — also the Mini window button in Chrome/Edge | `pip install adhush-0.8.0-py3-none-any.whl` |
...
```
python -m build                       # dist/adhush-0.8.0-py3-none-any.whl, .tar.gz
(cd platforms/web && zip -r ../../dist/adhush-web-0.8.0.zip .)
```
